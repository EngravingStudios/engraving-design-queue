...
const batchSplits = require("./batchSplits");
...
function isBatchStatus(statusValue) {
  return (cfg.statuses.batches || []).some((b) => b.value === statusValue);
}

/*
 * Group totals for the A/B split, keyed by the SAME grouping the queue
 * is already rendered by client-side — an order's first line's product
 * name (public/index.html's typeOrder/groupOrders) — not product_id,
 * so a "group" here always matches a visible section heading 1:1.
 */
function computeGroupTotals(orders) {
  const totals = new Map();
  for (const o of orders) {
    if (!o.lines.length) continue;
    const name = o.lines[0].productName;
...
async function fetchQueue(statusValue) {
  ...
  // A/B split only applies to batch tabs, not Label Hold/Pending (see
  // batchSplits.js). pruneIfStale catches a status whose old split has
  // fully turned over (see its comment); fillNew lazily locks in any
  // group that appeared since the split was started/last fetched — no
  // action needed from staff for orders added mid-morning.
  if (isBatchStatus(statusValue)) {
    const groupTotals = computeGroupTotals(orders);
    batchSplits.pruneIfStale(statusValue, groupTotals.map((g) => g.name));
    if (batchSplits.isActive(statusValue)) batchSplits.fillNew(statusValue, groupTotals);
    const splitGroups = batchSplits.getGroups(statusValue);
    for (const o of orders) {
      o.splitBucket = splitGroups[o.lines[0].productName] || null;
    }
  }
...
/*
 * Split state for every batch tab at once (not just the one currently
 * being viewed), so the split icon/pills can render correctly on
 * every tab, not only the active one. Cheap — batchSplits.isActive()
 * is a plain in-memory check, no DB round trip. Note: the staleness
 * self-heal (pruneIfStale, see batchSplits.js) only actually runs
 * when a status's queue is fetched, so a tab you haven't visited
 * could in theory still show "split" for a moment after it should
 * have self-healed — cosmetic only, it corrects itself the moment
 * that tab is opened.
 */
function splitStatusMap() {
  const map = {};
  for (const b of cfg.statuses.batches) map[b.value] = batchSplits.isActive(b.value);
  return map;
}

/*
 * Explicit "Split A/B" action. Fetches the batch fresh (which also
 * runs the staleness check above), then hands the current group
 * totals to batchSplits.start() — a no-op if a valid split is already
 * active, otherwise a fresh greedy balance across everything present
 * right now.
 */
async function splitBatch(statusValue) {
  if (!isBatchStatus(statusValue)) throw new Error("not a batch status");
  const orders = await fetchQueue(statusValue);
  const groupTotals = computeGroupTotals(orders);
  const started = batchSplits.start(statusValue, groupTotals);
  return { started };
}
...
async function moveBatch(fromStatus, force, user) {
  ...
  try {
    ...
    await conn.commit();
    // fromStatus's order set is about to be entirely replaced by
    // tomorrow's — any A/B split for it is done with (see
    // batchSplits.js; pruneIfStale() is the fallback for anything
    // that bypasses this, e.g. a crash mid-move).
    batchSplits.clearForStatus(fromStatus);
