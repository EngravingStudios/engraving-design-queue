...
/*
 * Collapse accidental double-spaces (fat-fingered spacebar) WITHOUT
 * touching deliberate wide spacing customers use as a design feature.
 * Two patterns are protected, line-by-line:
 *
 *   - Letter-spacing, e.g. "F  R  E  D  D  I  E" — if most of a line's
 *     space-separated tokens are single characters, the whole line is
 *     left untouched (it's a spaced-out word/name, not prose).
 *   - Decorative flanking, e.g. "-  I'm chipped  -" — a gap is left
 *     alone if the token on either side of it is pure
 *     punctuation/symbols (not alphanumeric), since that's a border/
 *     emphasis mark, not a typo.
 *
 * Everything else — an ordinary double space between two real words —
 * collapses to one. This is a heuristic, not a grammar check.
 */
const SYMBOL_TOKEN = /^[^a-zA-Z0-9]+$/;
function fixLineSpacing(line) {
  const parts = line.match(/\S+|\s+/g);
  if (!parts) return line;

  const wordTokens = parts.filter((p) => !/^\s/.test(p));
  const singleCharCount = wordTokens.filter((w) => w.length === 1).length;
  const isLetterSpaced =
    wordTokens.length >= 3 && singleCharCount / wordTokens.length >= 0.6;
  if (isLetterSpaced) return line;

  return parts
    .map((part, i) => {
      if (!/^\s{2,}$/.test(part)) return part;
      const before = parts[i - 1] || "";
      const after = parts[i + 1] || "";
      const decorative = SYMBOL_TOKEN.test(before) || SYMBOL_TOKEN.test(after);
      return decorative ? part : " ";
    })
    .join("");
}
